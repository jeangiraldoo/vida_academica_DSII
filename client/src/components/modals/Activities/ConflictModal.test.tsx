...
import ConflictModal, { type ConflictModalItem, type ConflictModalSubtask } from "./ConflictModal";
...
function renderModal(opts: Opts = {}) {
	...
	const onChangeDate =
		"onChangeDate" in opts ? opts.onChangeDate : vi.fn().mockResolvedValue(undefined);
	const onReduceHours =
		"onReduceHours" in opts ? opts.onReduceHours : vi.fn().mockResolvedValue(undefined);
...
describe("ConflictModal — rendering", () => {
	...
	it("renders the backdrop", () => {
		renderModal();
		expect(screen.getByTestId("conflict-modal-backdrop")).toBeInTheDocument();
	});
	...
	it("renders the [X] close button with accessible label", () => {
		renderModal();
		expect(screen.getByLabelText("Cerrar modal de conflictos")).toBeInTheDocument();
	});
	...
	it("shows the empty-state when conflicts is an empty array", () => {
		...
		expect(screen.getByText(/No hay sobrecargas ni fechas en riesgo/)).toBeInTheDocument();
	});

	it("does NOT show the empty-state when there are conflicts", () => {
		renderModal();
		expect(screen.queryByText("Todo está en orden")).not.toBeInTheDocument();
	});
});
...
describe("ConflictModal — expand/collapse", () => {
	it("auto-expands the first conflict item on mount", () => {
		...
		expect(screen.getByTestId("conflict-item-toggle-btn-1").textContent).toContain("Ocultar");
	});

	it("collapses an expanded item when its toggle is clicked", () => {
		renderModal();
		fireEvent.click(screen.getByTestId("conflict-item-toggle-btn-1"));
		expect(screen.getByTestId("conflict-item-toggle-btn-1").textContent).toContain("Resolver");
	});
	...
	it("shows subtask rows when the item is expanded", () => {
		renderModal();
		expect(screen.getByTestId("conflict-subtask-row-101")).toBeInTheDocument();
		expect(screen.getByTestId("conflict-subtask-row-102")).toBeInTheDocument();
	});
	...
	it("shows the no-subtasks message when a conflict has an empty subtasks array and is expanded", () => {
		...
		expect(screen.getByText(/No se encontraron subtareas detalladas/)).toBeInTheDocument();
	});
});
...
describe("ConflictModal — Change Date flow", () => {
	function openDateResolver() {
		renderModal();
		fireEvent.click(screen.getByTestId("conflict-subtask-change-date-btn-101"));
	}

	it("opens the resolver card when 'Cambiar fecha' is clicked", () => {
		openDateResolver();
		expect(screen.getByTestId("conflict-resolver-card")).toBeInTheDocument();
	});

	it("shows title 'Cambiar fecha de subtarea' in the resolver", () => {
		openDateResolver();
		expect(screen.getByText("Cambiar fecha de subtarea")).toBeInTheDocument();
	});
	...
	it("pre-fills the date input with the subtask targetDate", () => {
		openDateResolver();
		expect(screen.getByTestId("conflict-resolver-date-input")).toHaveValue("2099-07-10");
	});
	...
	it("calls onChangeDate with the correct payload when 'Guardar fecha' is clicked", async () => {
		...
		fireEvent.click(screen.getByTestId("conflict-subtask-change-date-btn-101"));
		fireEvent.change(screen.getByTestId("conflict-resolver-date-input"), {
	...
	it("shows a validation error when the date field is empty on save", async () => {
		renderModal();
		fireEvent.click(screen.getByTestId("conflict-subtask-change-date-btn-101"));
		fireEvent.change(screen.getByTestId("conflict-resolver-date-input"), { target: { value: "" } });
		fireEvent.click(screen.getByTestId("conflict-resolver-save-btn"));

		await waitFor(() =>
			expect(screen.getByText("Selecciona una fecha valida.")).toBeInTheDocument(),
		);
	});

	it("shows a server error when onChangeDate rejects", async () => {
		const onChangeDate = vi.fn().mockRejectedValue(new Error("Network error"));
		renderModal({ onChangeDate });
		...
		await waitFor(() =>
			expect(
				screen.getByText("No se pudo actualizar la fecha. Intenta de nuevo."),
			).toBeInTheDocument(),
		);
	});
	...
	it("closes the resolver (not the whole modal) after a successful save", async () => {
		...
		await waitFor(() =>
			expect(screen.queryByTestId("conflict-resolver-card")).not.toBeInTheDocument(),
		);
	...
	it("shows suggested future idle date when conflict is overdue", () => {
		...
		fireEvent.click(screen.getByTestId("conflict-subtask-change-date-btn-101"));
		expect(screen.getByTestId("conflict-resolver-suggest-date-btn")).toBeInTheDocument();
	});
...
describe("ConflictModal — Adjust Hours flow", () => {
	function openHoursResolver() {
		renderModal();
		fireEvent.click(screen.getByTestId("conflict-subtask-adjust-hours-btn-101"));
	}
	...
	it("shows title 'Reducir horas de subtarea' in the resolver", () => {
		openHoursResolver();
		expect(screen.getByText("Reducir horas de subtarea")).toBeInTheDocument();
	});
	...
	it("pre-fills the hours input with the subtask estimatedHours", () => {
		openHoursResolver();
		expect(screen.getByTestId("conflict-resolver-hours-input")).toHaveValue(3);
	});
	...
	it("clicking the hours suggestion chip applies the suggested value", () => {
		...
		const chip = screen.getByTestId("conflict-resolver-suggest-hours-btn");
		fireEvent.click(chip);
		const input = screen.getByTestId("conflict-resolver-hours-input") as HTMLInputElement;
	...
	it("calls onReduceHours with the correct payload on save", async () => {
		...
		fireEvent.click(screen.getByTestId("conflict-subtask-adjust-hours-btn-101"));
		fireEvent.change(screen.getByTestId("conflict-resolver-hours-input"), {
			target: { value: "2" },
		});
		fireEvent.click(screen.getByTestId("conflict-resolver-save-btn"));
	...
	it("shows a validation error when hours value is 0 (< 1)", async () => {
		...
		await waitFor(() =>
			expect(screen.getByText("Ingresa horas validas (1 o mas).")).toBeInTheDocument(),
		);
	});
	...
	it("shows a server error when onReduceHours rejects", async () => {
		const onReduceHours = vi.fn().mockRejectedValue(new Error("Server error"));
		renderModal({ onReduceHours });
		...
		await waitFor(() =>
			expect(
				screen.getByText("No se pudieron actualizar las horas. Intenta de nuevo."),
			).toBeInTheDocument(),
		);
	});
	...
	it("does NOT show the hours suggestion chip when estimatedHours <= 1", () => {
		...
		fireEvent.click(screen.getByTestId("conflict-subtask-adjust-hours-btn-101"));
		expect(screen.queryByTestId("conflict-resolver-suggest-hours-btn")).not.toBeInTheDocument();
	});
});
...
describe("ConflictModal — footer", () => {
	it("renders the footer status message", () => {
		renderModal();
		expect(screen.getByText("Puedes resolver ahora o volver luego.")).toBeInTheDocument();
	});
});
...
describe("ConflictModal — Tour callback coverage", () => {
	it("notifies when the tour is finished or skipped", () => {
		...
		const props = (window as unknown as { __lastJoyrideProps: Record<string, unknown> })
			.__lastJoyrideProps;
		if (props) {
			const handler = (props.onEvent || props.callback) as (data: { status: string }) => void;
			if (handler) {
				act(() => {
